...
### 6.6 Farmer Chatbot Web Interface & Chat History UX (Steps 5F & 5G)
...
- **Session Flow & History UX**:
  - Automatically loads previous chat sessions via `GET /api/v1/chat/sessions` with most recent updated first.
  - **Active Session Persistence**: Restores active conversation on page reload using `sessionStorage` with server-side validation.
  - **Deferred Session Creation**: "+ New Chat" resets the interface to a welcome screen; backend session is created only when the first question is sent.
  - **Smart Title Generation**: Automatically names sessions from the farmer's initial inquiry rather than generic timestamps.
  - **Real-Time Client Search**: Real-time filtering by conversation title and language in the sidebar.
  - **Race Condition Prevention**: Prevents out-of-order message rendering during rapid session switching.
- **Interactive Features**:
  - Shimmer skeleton loaders during network fetches and retry buttons on failures.
